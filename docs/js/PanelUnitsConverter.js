...
export {getConvertData, convertValue, UnitsConverterPanel};
...
const convertDataLength=[];
const convertDataArea=[];
const convertDataVolume=[];
const convertDataVelocity=[];
const convertDataPower=[];
const convertDataEnergy=[];
const convertDataTemperature=[];
...
function initConverter() {
  if (convertDataLength.length>0) return;
  ...
  convertDataLength.push(...[
    ...
    {name: language.units.lengthAngstroem, unit: '\u212B', factor: 1E-10, wiki: language.units.lengthAngstroemWiki, moreNames: ["Angström","Angstroem"]},
    {name: language.units.lengthFeet, unit: 'ft', factor: 0.3048, wiki: language.units.lengthFeetWiki},
    {name: language.units.lengthInch, unit: 'in', factor: 0.0254, wiki: language.units.lengthInchWiki, moreNames: ["inch","Zoll"]},
    {name: language.units.lengthSeaMiles, unit: "sm", factor: 1852, wiki: language.units.lengthSeaMilesWiki, moreNames: ["Seemeilen","Sea miles","Nautical miles"]},
    {name: language.units.lengthPoints, unit: "pt" , factor: 1/72.27*0.0254, wiki: language.units.lengthPointsWiki, moreNames: ["Punkte","Punkt","Point","Points"]},
  ...
  convertDataVolume.push(...[
    {name: language.units.volumeCubicMeters, unit: "m<sup>3</sup>", initialValue: 1, moreNames: ["m3","m³"]},
    {name: language.units.volumeCubikMillimeters, unit: "mm<sup>3</sup>", calcToBase: x=>x/1E9, calcFromBase: x=>x*1E9, moreNames: ["mm3","mm³"]},
    {name: language.units.volumeCubikKilometers, unit: "km<sup>3</sup>", calcToBase: x=>x*1E9, calcFromBase: x=>x/1E9, moreNames: ["km3","km³"]},
    {name: language.units.volumeLiters, unit: "l", factor: 1/1000},
    {name: language.units.volumeGalons, unit: "gal" ,factor: 0.0037854111013237, wiki: language.units.volumeGalonsWiki, moreNames: ["Galone"]},
    ...
    {name: language.units.volumeOkerReservoirs ,factor: 46850000, wiki: language.units.volumeOkerReservoirsWiki, moreNames: ["Okerstausee","Okertalsperre","Okertalsperren"]},
    {name: language.units.volumeCups, factor: 0.24, wiki: language.units.volumeCupsWiki, moreNames: ["Tasse"]},
  ...
  convertDataVelocity.push(...[
    {name: language.units.velocityKmh, unit: "km/h", initialValue: 1, moreNames: ["kmh"]},
    {name: language.units.velocityMs, unit: language.units.velocityMsUnit, factor: 3.6, moreNames: ["ms"]},
    {name: language.units.velocityMih, unit: "miles/h", factor: 1.609344, moreNames: ["mih","mi/h"]},
    ...
    {name: language.units.velocityWarpFactor, calcToBase: x=>warpTokmh(x), calcFromBase: x=>kmhToWarp(x), wiki: language.units.velocityWarpFactorWiki,moreNames: ["Warpfaktor","Warp"]}
  ]);

  convertDataPower.push(...[
    {name: language.units.powerW, unit: "W", initialValue: 1, wiki: language.units.powerWWiki},
    {name: language.units.powerPS, unit: language.units.powerPSUnit, factor: 735.49875, wiki: language.units.powerPSWiki, moreNames: ["PS"]},
  ]);

  convertDataEnergy.push(...[
  ...
  convertDataWeightAndMass.push(...[
    ...
    {name: language.units.WeightAndMassTons, unit: "t", factor: 1000, moreNames: ["Tonne"]},
    ...
    {name: language.units.WeightAndMassOunces, factor: 0.031103473480401, wiki: language.units.WeightAndMassOuncesWiki, moreNames: ["Unze"]},
    {name: language.units.WeightAndMassCarat, factor: 0.2/1000, wiki: language.units.WeightAndMassCaratWiki}
  ]);

  convertDataAngle.push(...[
    {name: language.units.angleDEG, unit: '° DEG', initialValue: 360, wiki: language.units.angleDEGWiki, moreNames: ["DEG"]},
...
function convertValue(value, oldUnit, newUnit) {
  ...
  if (oldUnit.factor) value=value*oldUnit.factor;
  if (oldUnit.calcToBase) value=oldUnit.calcToBase(value);

  if (newUnit.factor) value=value/newUnit.factor;
  if (newUnit.calcFromBase) value=newUnit.calcFromBase(value);
...
class UnitsConverterPanel extends Panel {
  ...
  _firstShow() {
    ...
    /* Panels */
    initConverter();
    this.#buildUnitsPanel(language.units.length,convertDataLength);
    this.#buildUnitsPanel(language.units.area,convertDataArea);
    this.#buildUnitsPanel(language.units.volume,convertDataVolume);
    this.#buildUnitsPanel(language.units.velocity,convertDataVelocity);
    this.#buildUnitsPanel(language.units.power,convertDataPower);
    this.#buildUnitsPanel(language.units.energy,convertDataEnergy);
    this.#buildUnitsPanel(language.units.temperature,convertDataTemperature);
